-- Migration: Add customer_balances view and partial down-payment support to record_sale
...
select
  ...
  sum(s.total - s.amount_paid) as total_debt,
  count(s.id) as unpaid_sales_count,
  max(s.sold_at) as latest_sale_at
from public.sales s
left join public.customers c on c.id = s.customer_id
where s.total > s.amount_paid
group by s.business_id, s.customer_id, c.name, c.phone;

grant select on public.customer_balances to authenticated;

-- Drop old 7-parameter function signature to resolve PostgreSQL ambiguity
...
begin
  ...
  if p_amount_paid is not null then
    v_amount_paid := greatest(0, least(v_subtotal, p_amount_paid));
